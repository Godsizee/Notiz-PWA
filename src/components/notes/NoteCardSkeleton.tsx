...
export default function NoteCardSkeleton({ lines = 3 }: NoteCardSkeletonProps) {
  return (
    <div className="mb-4 break-inside-avoid rounded-3xl p-5 border border-[var(--border-color)] bg-[var(--card-bg)] shadow-sm overflow-hidden">
      {/* Title placeholder */}
      <div className="h-4 w-2/3 rounded-full bg-[var(--text-muted)]/15 animate-pulse mb-4" />
      ...
      <div className="space-y-2.5">
        {Array.from({ length: lines }).map((_, i) => (
          <div
            key={i}
            className="h-3 rounded-full bg-[var(--text-muted)]/10 animate-pulse"
            style={{ width: `${90 - i * 15}%` }}
          />
        ))}
      </div>
